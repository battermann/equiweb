module Bootstrap.Alt.Alert exposing
...
import Bootstrap.Alt.Internal.Role as Role exposing (Role(..))
...
maybeAddDismissButton visibilty configRec children_ =
    if isDismissable configRec then
        injectButton
            (Html.button
                ([ type_ "button", class "btn-close", attribute "aria-label" "close" ]
